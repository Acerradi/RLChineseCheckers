...
# Discounted return computation (n-step TD)
GAMMA = 0.99

# Resign logic: if predicted value falls below threshold after MIN_RESIGN_MOVES moves, resign
RESIGN_ENABLED = True
...
def bootstrap_imitation(num_games: int = 500,
    ...
    for local_game_idx in pbar:
        ...
        pbar.set_postfix({"loss": loss_str, "buf": len(buffer), "trunc": truncated})
        tqdm_write(f"[bootstrap] game={game_idx} moves={env.game.move_count} "
                   f"examples={len(examples)} buf={len(buffer)} "
                   f"loss={loss_str} trunc={truncated} t={elapsed:.1f}s")
...
def self_play_refinement(start_checkpoint: str | None = None,
    ...
    for local_game_idx in pbar:
        ...
        while not done and env.game.move_count < max_moves_per_game:
            ...
            if colour == learner_colour:
                chosen_action, gs, target_policy, mcts_meta = mcts_label_for_observation(
                    learner=learner,
                    env=env,
                    colour=colour,
                ...
                if (RESIGN_ENABLED
                        ...
                        and env.game.move_count >= MIN_RESIGN_MOVES):
                    with torch.no_grad():
                        learner.model.eval()
                        gs_dev = learner._graph_to_device(gs)
                        _, pred_value = learner.model(gs_dev)
                        learner.model.train()
                    if float(pred_value.item()) < RESIGN_THRESHOLD:
                        resigned = True
                        break
                ...
                step_result = env.step(colour, chosen_action)
                # Store the shaping reward so discounted returns can use it
                per_colour_examples[colour][-1]["step_reward"] = float(step_result.reward)
                done = step_result.done
            else:
                chosen_action = policy_cache[colour].select_action(obs)
                step_result = env.step(colour, chosen_action)
        ...
        pbar.set_postfix({"loss": loss_str,
                          "buf": len(buffer),
                          "adj": adjudication_type,
                          "resign": run_metrics["resigned"]})
        tqdm_write(f"[self-play] game={game_idx} moves={env.game.move_count} "
                   f"examples={len(examples)} buf={len(buffer)} "
